...
function mostrarProductos(lista, esCarrito=false) {
    ...
    lista.forEach(p => {
        ...
        let cantidad = carrito[p.nombre] || 0;
        contenedor.innerHTML += `
          <div class="col-md-4">
    ...
    for (const nombre in carrito) {
      ...
      <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap">
        <div class="me-auto">
          ...
          Cantidad: <span id="cantidad-${nombre}">${cantidad}</span> |
...
function incrementarCantidad(nombre) {
  carrito[nombre]++;
  actualizarCarrito();
  verCarrito(); // para refrescar la vista del carrito con la nueva cantidad
}
...
function disminuirCantidad(nombre) {
  if (carrito[nombre] > 1) {
    ...
    delete carrito[nombre]; // si llega a 0, se elimina del carrito
  }
  actualizarCarrito();
  verCarrito();
}
...
function agregarAlCarrito(nombre) {
    ...
    if (!isNaN(cantidad) && cantidad > 0) {
        ...
        actualizarCarrito();
         const cantidadTexto = document.querySelector(`#cantidad-carrito-${nombre}`);
         if (cantidadTexto) {
            cantidadTexto.textContent = `Cantidad en carrito: ${carrito[nombre]}`;
          }
    }
